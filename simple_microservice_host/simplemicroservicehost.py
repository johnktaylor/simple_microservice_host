...
class SimpleMicroserviceHost:
    ...
    def handle_message(self, messagebytes: bytes) -> Dict[str, Any]:
        ...
        try:
            ...
            logging.error(f"Host - Error deserializing message: {e}")
            raise e
        ...
        if not self.settings.get('cryptography').get('disable_signature_verification'):
            try:
                if not self.servermessageprocessing.verify_signature(message.get('signature'), message):
                    logging.error(f"Host - Signature verification failed")
                    return self.servermessageprocessing.generate_response(
                        message, 
                        message.get("operation", "unknown"), 
                        "error", 
        ...
        if message.get('encrypt'):
            logging.info(f"Host - Encrypt flag is true")
            encrypted_data = base64.b64decode(message.get('data'))
            try:
                algorithm = message.get("algorithm")
                if not algorithm:
                    logging.error(f"Host - Algorithm not found")
                ...
                if algorithm not in self.servermessageprocessing.get_encryption_suites(self.settings):
                    logging.error(f"Host - Algorithm not supported")
                ...
                logging.info(f"Host - Decrypted data")
                message['data'] = self.messageserialization.deserialize(decrypted_data)
            except Exception as e:
                logging.error(f"Host - Decryption failed: {e}")
        ...
        if cryptography_settings.get('enforce_encryption')==True:
            logging.info(f"Host - Enforce encryption is true")
            if not message.get('encrypt'):
                logging.error(f"Host - Encrypt flag is false and enforce encryption is true")
        ...
        try:
            ...
            logging.info(f"Host - Timestamp parsed")
        except ValueError:
            logging.error(f"Host - Invalid timestamp format")
            return {"status": "error", "message": "Invalid timestamp format"}

        try:
            operation = message.get('operation')
            logging.info(f"Host - Operation: {operation}")
            data = message.get('data', {})
            ...
            if operation == 'request_server_encryption_suites':
                ...
                logging.info(f"Host - Response After Request Encryption Suites: {type(response)}")
            elif operation == 'key_exchange_request':
                response = self.servermessageprocessing.handle_key_exchange_request(data, message)
                logging.info(f"Host - Response After Key Exchange Request: {type(response)}")
            else:
                logging.info(f"Host - Passing Operation To Extended Functions: {operation}")
                response = self.extendedfunctions.handle_message(message)
                logging.info(f"Host - Response After Extended Functions: {type(response)}")

        except Exception as e:
            logging.error(f"Host - Error handling message: {e}")
        ...
        logging.info(f"Host - Response Before Encryption: {response}")
        
        if message.get('encrypt'):
            logging.info(f"Host - Encrypt flag is true")
            response_data = self.messageserialization.serialize(response.get('data'))
            logging.info(f"Host - Response data: {response_data}")
            ...
            logging.info(f"Host - Encrypted response data")
